        ...
        {!! Form::open(['method' => 'POST', 'id' => 'printForm']) !!}
            ...
            <table id="table" class="table table-sm table-bordered" style="margin-top:30px;text-align:center;font-size:85%">
                ...
                <tbody>
                    ...
                    <ul class="actions">
                        @can('all', $users[0])
                            <li><a href="#" onclick="deleteUsers(true)">حذف کاربران</a></li>
                        @endcan 
                    </ul>
...
@section('scripts')
    <script>
        ...
        function deleteUsers(multiple = false) {
            var form = document.getElementById("printForm");

            if (multiple) {
                var respond = confirm('آیا از حذف کاربران انتخاب شده مطمئن هستید ؟');
            }
        ...
        function contextMenu(event) {
            ...
            if (document.getElementById('edit')) {
                document.getElementById('edit').onclick = (event) => {
                    window.location.href = 'users/' + item.firstChild.innerHTML + '/edit';
                }
            }
